  ...
  Future<void> _handleWebRedirect() async {
    try {
      ...
      if (cred != null && cred.user != null && mounted) {
        Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => MainScreen(userEmail: cred.user!.email ?? 'Unknown')));
        return;
      }
    } catch (e) {
    ...
    if (FirebaseAuth.instance.currentUser != null && mounted) {
      Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => MainScreen(userEmail: FirebaseAuth.instance.currentUser!.email ?? 'Unknown')));
      return;
    }
  ...
  Future<void> _signInWithGoogle(BuildContext sheetContext) async {
    try {
      ...
      if (mounted && cred.user != null) {
        ...
        Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => MainScreen(userEmail: identifier)));
      }
    } catch (e) {

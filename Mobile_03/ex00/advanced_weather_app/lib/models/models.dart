class LocationResult {
  ...
  LocationResult({required this.name, required this.region, required this.country, required this.latitude, required this.longitude});
...
class WeatherData {
  ...
  factory WeatherData.fromJson(Map<String, dynamic> json) {
    ...
    final winds = hourlyJson['wind_speed_10m'] ?? [];
    final codes = hourlyJson['weather_code'] ?? [];
    
    List<HourlyWeather> hourlyList = [];
    int hourlyCount = times.length < 24 ? times.length : 24;
    for (int i = 0; i < hourlyCount; i++) {
      hourlyList.add(HourlyWeather(time: times[i], temperature: (temps[i] as num?)?.toDouble() ?? 0.0, windSpeed: (winds[i] as num?)?.toDouble() ?? 0.0, weatherCode: codes[i] ?? 0));
    }

    final dailyJson = json['daily'] ?? {};
    final dTimes = dailyJson['time'] ?? [];
    final dMax = dailyJson['temperature_2m_max'] ?? [];
    final dMin = dailyJson['temperature_2m_min'] ?? [];
    final dCodes = dailyJson['weather_code'] ?? [];

    List<DailyWeather> dailyList = [];
    int dailyCount = dTimes.length < 7 ? dTimes.length : 7;
    for (int i = 0; i < dailyCount; i++) {
      dailyList.add(DailyWeather(date: dTimes[i], maxTemp: (dMax[i] as num?)?.toDouble() ?? 0.0, minTemp: (dMin[i] as num?)?.toDouble() ?? 0.0, weatherCode: dCodes[i] ?? 0));
    }
